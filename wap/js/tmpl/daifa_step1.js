$(function() {
	...
	var changeAddress_es = function(key, area_id, city_id, freight_hash) {
		$.ajax({//保存收货地址
			type:'post',
			url:ApiUrl+'/index.php?act=member_buy&op=change_address_daifa',
			data:{key:key,area_id:area_id,city_id:city_id,freight_hash:freight_hash},
			dataType:'json',
			success:function(result){
				var data = result.datas;
	...
	$.ajax({//提交订单信息
		type:'post',
		url:ApiUrl+'/index.php?act=member_buy&op=daifa_step1',
		dataType:'json',
		data:data,
		success:function(result){
			var data = result.datas;
...
*/
	...
	//新增代发地址
	for(var i=0; i<area_list.length; i++){
		$("#area_selects").find('select').append('<option value="' + area_list[i].area_id + '">' + area_list[i].area_name + '</option>');
	}
	
	$('#area_selects').on('change', 'select', function(){
		$('#area_ids').val($(this).val());
		var area_ids = $("input[name=area_ids]").val();
        var city_ids = $("input[name=area_ids_2]").val();
        var freight_hash = $("input[name=freight_hash]").val();
		changeAddress_es(key, area_ids, city_ids, freight_hash);
		var _index = $(this).index();
		$('#area_ids_' + (_index + 1)).val($(this).val());
		var area_list = get_area(key, $(this).val());
		if (area_list.length == 0) {
			return false;
		}
		for (var i=_index+1; i<=4; i++) {
			$('#area_ids_' + (i + 1)).val('');
			$("#area_selects").find('select').eq(_index+1).remove();
		}
		
		$("#area_selects").append('<select class="select-30"></select>');
		$("#area_selects").find('select').last().append('<option value="">请选择...</option>');
		for (var i=0; i<area_list.length; i++) {
			$("#area_selects").find('select').last().append('<option value="' + area_list[i].area_id + '">' + area_list[i].area_name + '</option>');
		}
	});
    ...
    $(".save-address").click(function (){//更换收获地址
        ...
        if(address_id>0){//变更地址
			...
			if($.sValid()){
				...
				if ($('#area_selects').find('select').last().val() == '') {
                    $(".error-tips").html('请选择到最后一级地区').show();
                    return false;
                }
				var area_info = '';
				if (typeof($('#area_selects > select').eq(0)[0]) != 'undefined') {
					area_info += $('#area_selects > select').eq(0)[0].options[$('#area_selects > select').eq(0)[0].selectedIndex].innerHTML;
				}
				if (typeof($('#area_selects > select').eq(1)[0]) != 'undefined') {
					area_info += ' ' + $('#area_selects > select').eq(1)[0].options[$('#area_selects > select').eq(1)[0].selectedIndex].innerHTML;
				}
				if (typeof($('#area_selects > select').eq(2)[0]) != 'undefined') {
					area_info += ' ' + $('#area_selects > select').eq(2)[0].options[$('#area_selects > select').eq(2)[0].selectedIndex].innerHTML;
				}
				if (typeof($('#area_selects > select').eq(3)[0]) != 'undefined') {
					area_info += ' ' + $('#area_selects > select').eq(3)[0].options[$('#area_selects > select').eq(3)[0].selectedIndex].innerHTML;
				}
...
*/
		...
		//代发订单地址获取 QQ：410668163
		data.to_name = $('input[name=to_name]').val();
		data.to_mob_phone = $('input[name=to_mob_phone]').val();
		data.to_tel_phone = $('input[name=to_tel_phone]').val();
		area_ids_1 = $('input[name=area_ids_1]').val();
		area_ids_2 = $('input[name=area_ids_2]').val();
		area_ids_3 = $('input[name=area_ids_3]').val();
		area_ids_4 = $('input[name=area_ids_4]').val();
		data.area_ids = area_ids_1+' '+area_ids_2+' '+area_ids_3+' '+area_ids_4;
		data.to_address = $('input[name=to_address]').val();
		if (data.to_name == '') {
			alert('请填写收货人');
			return false;
		};
		if (data.to_mob_phone == '' && data.to_tel_phone == '') {
			alert('请填写收货人联系方式');
			return false;
		};
		if (data.area_ids == '') {
        ...
        $.ajax({
        	...
        	success:function(result){
        		...
        		if(result.datas.pay_sn != ''){
        			if(result.datas.order_daifa == 2){
						location.href = WapSiteUrl+'/tmpl/member/daifa_list.html';
        			}else{
